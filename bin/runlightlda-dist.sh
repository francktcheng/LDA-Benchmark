...
runlightlda()
{
    ...
    if [ -f $logfile ] ; then
        ...
        machfile=cluster-"$nodes".ip
        if [ -f $machfile ] ; then
        # run it
        cmd="MV2_ENABLE_AFFINITY=$MV2_ENABLE_AFFINITY; mpirun -machinefile $machfile $homedir/bin/$lightldabin -num_vocabs $num_vocabs -num_topics $topic -num_servers $nodes -num_iterations $iter -alpha $alpha -beta $beta -mh_steps $mh_step -num_local_workers $threads -num_blocks 1 -max_num_document $max_doc -input_dir $datadir -data_capacity $data_cap -model_capacity $model_cap -delta_capacity $delta_cap"
        echo $cmd |tee $logfile
#export MV2_ENABLE_AFFINITY=0
        mpirun -machinefile $machfile $homedir/bin/$lightldabin -num_vocabs $num_vocabs -num_topics $topic -num_servers $nodes -num_iterations $iter -alpha $alpha -beta $beta -mh_steps $mh_step -num_local_workers $threads -num_blocks 1 -max_num_document $max_doc -input_dir $datadir -data_capacity $data_cap -model_capacity $model_cap -delta_capacity $delta_cap -alias_capacity $alias_cap |tee -a $logfile
        else
            echo "$machfile not exist, skip this test"
        fi
    fi
}
