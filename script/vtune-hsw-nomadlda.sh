...
dlimit=0 
path_nomadlda=/scratch/fg474admin/LDA-langshi-Exp/ldaprofiling/src/nomadlda
path_tbb=/opt/intel/compilers_and_libraries_2016/linux/tbb/lib/intel64_lin/gcc4.4
src_path_nomadlda=/scratch/fg474admin/LDA-langshi-Exp/ldaprofiling/src/nomadlda
...
## result path
obj=R-$Arch-$Name-thd$thread-$action-$type-multiNode
...
# launch the program
../bin/profile_nomadlda.sh $thread 10 &
exec_pid=$!

# sleep 10

## get the process name
# pid=$(ps -u fg474admin | grep "f+nomad-lda.noe" | awk '{ print $1 }')
...
amplxe-cl -$action $type \
    ...
    -duration=$sec \
    -r $resDir \
    -search-dir $path_nomadlda \
    -search-dir $path_tbb \
    -source-search-dir $src_path_nomadlda \
